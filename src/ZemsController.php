...
class ZemsController extends Controller
{
    ...
    public function zems_get($data = false)
    {
        ...
        if(!$json_data->isEmpty()){
            if($this->request->route()->getPrefix() == '/gui'){                
    ...
    public function zems_store($data = false)
    {
        ...
        if($store->save()){
            ...
            if($this->request->route()->getPrefix() == '/gui'){   
                ...
                return redirect("/gui/".$current_route)->with(['msg' => 'Data Insterted Successfully']);                
            }else{
                return $json_data;
            }
        } else {
    ...
    public function zems_update($data = false)
    {
        ...
        if($store->save()){
            if($this->request->route()->getPrefix() == '/gui'){   
                $current_route = $this->request->route()->getName();
                return redirect("/gui/".$current_route)->with(['msg' => 'Data Updated Successfully']);                
            }else{
                return $json_data;
            }
        } else {
    ...
    public function zems_delete($data = false)
    {
        ...
        if($store->delete()){
            ...
            if($this->request->route()->getPrefix() == '/gui'){   
                $current_route = $this->request->route()->getName(); 
                return redirect("/gui/".$current_route)->with(['msg' => 'Data Deleted Successfully']);                
            }else{
                return $json_data;
            }
        } else {
    ...
    public function zems_gui($data = false)
    {
        ...
        $json_data = $this->zems_get($data);
        return View::make($data['view'], compact('json_data', 'data'));
    }
